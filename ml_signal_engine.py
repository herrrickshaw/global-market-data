...
class MLSignalEngine:
    ...
    def _make_sequences(self, features: pd.DataFrame, target: pd.Series) -> tuple:
        """
    ...
    def predict_batch(self, ohlc_dict: dict, max_workers: int = None) -> pd.DataFrame:
        """
        ...
        n = len(ohlc_dict)
        print(
            f"  ML signal engine: predicting {n} stocks "
        ...
        with ThreadPoolExecutor(max_workers=n_workers) as pool:
            futures = {pool.submit(self.predict, sym, df): sym for sym, df in ohlc_dict.items()}
    ...
    def evaluate_on_backtest(self, ohlc_dict: dict, test_start: str, test_end: str) -> dict:
        """
        ...
        print(f"  RMSE:             {rmse:.4f}  " f"(paper: LR 0.304, LSTM 0.346, RNN 0.415)")
        print(f"  MAE:              {mae:.4f}  " f"(paper: LR 0.207, LSTM 0.237, RNN 0.329)")
        print(
            f"  Directional Acc:  {hit:.1f}%  " f"(>50% = better than random; paper reports 76–85%)"
        )
...
def compare_models(df_ohlc: pd.DataFrame, symbol: str = "TEST") -> pd.DataFrame:
    ...
    print(f"\n  Model comparison for {symbol} " f"(train={split} rows, test={len(X_test)} rows):")
    print(
        f"  {'Model':<25} {'Train RMSE':>11} {'Train MAE':>10} "
        f"{'Test RMSE':>10} {'Test MAE':>9}"
    )
    print("  " + "─" * 65)
    for _, r in df.iterrows():
        print(
            f"  {r['Model']:<25} {r['Train_RMSE']:>11.4f} {r['Train_MAE']:>10.4f} "
            f"{r['Test_RMSE']:>10.4f} {r['Test_MAE']:>9.4f}"
        )
    print(
        "  Paper benchmark:          LR: 0.334 train / 0.304 test  "
        "(LSTM: 0.355 / 0.346, RNN: 0.383 / 0.415)"
    )
