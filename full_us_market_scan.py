...
def _save_symbol_cache(nasdaq_syms, nyse_syms):
    try:
        _SYMBOL_CACHE.write_text(
            json.dumps({"ts": time.time(), "nasdaq": nasdaq_syms, "nyse": nyse_syms})
        )
    except Exception:
        pass
...
def fundamental_scan(symbol: str) -> dict:
    ...
    if inc is not None:
        ...
        out["qualifies_mf"] = bool(
            roic and roic > 25 and ey and ey > 15 and bv and bv > 0 and mcap > 50e6
        )
    ...
    if inc_q is not None and len(inc_q.columns) >= 5:
        ...
        out["qualifies_bc"] = bool(
            sales_g and sales_g > 15 and profit_g and profit_g > 20 and ni_usd_m and ni_usd_m > 1
        )  # >$1M net profit
...
def main(
    ...
    print(f"\n{'#'*60}")
    print("  FULL US MARKET SCAN — NASDAQ + NYSE")
    print(f"  Started: {datetime.now().strftime('%d %b %Y  %H:%M:%S')}")
    print(f"  Cache: {'✅ active (Parquet)' if _CACHE_OK else '⚠️  disabled'}")
    print(
        f"  ML signal: {'✅ Ridge regression (AlQahtani et al. 2025)' if _ML_OK else '⚠️  disabled'}"
    )
    print(f"{'#'*60}\n")
    ...
    if _NSE_OK:
        try:
            ...
            if not sp500.empty:
                ...
                print(
                    f"  S&P 500: {sp_last:,.0f}  |  200 DMA: {sp_dma200:,.0f}  "
                ...
                try:
                    ...
                    if not vix.empty:
                        ...
                        vix_lvl = (
                            "NORMAL" if vix_val < 18 else "ELEVATED" if vix_val < 25 else "PANIC"
                        )
            ...
            for future in as_completed(futures):
                ...
                try:
                    ...
                    if done % 200 == 0 or done == len(all_syms_for_fund):
                        print(
                            f"    {done}/{len(all_syms_for_fund)} done  "
    ...
    if _ML_OK and ohlc_data:
        ...
        print(
            f"  ML: BULLISH={n_bull} | BEARISH={n_bear} | "
    ...
    if six_screen_rows:
        ...
        for r in top:
            print(
                f"    {r['Symbol']:<10} {r.get('Name',''):<30} {r['Screens_Passed']}/6 "
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--top", type=int, default=0, help="Limit scan to first N symbols (0 = all)"
    )
    parser.add_argument(
        "--no-scans",
        action="store_true",
