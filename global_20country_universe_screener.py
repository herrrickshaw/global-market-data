...
class Global20CountryScreener:
    ...
    def generate_global_universe(self) -> list:
        ...
        print("   Regions: 4 (North America, Europe, Asia-Pacific, Emerging Markets)")
    ...
    def _stage1_filter(self, companies: list) -> list:
        ...
        for company in companies:
            if (
                company.get("debt_to_equity", 0) <= 2.0
                and company.get("capex_to_revenue", 0) >= 0.005
                and (company.get("avg_fcf", 0) or 0) >= 0
                and (company.get("ni_margin_change", 0) or 0) >= -5
            ):
    ...
    def _stage2_filter(self, companies: list) -> list:
        ...
        for company in companies:
            if (
                (company.get("debt_service_coverage", 1.5) or 1.5) >= 1.0
                and (company.get("interest_coverage", 5) or 5) >= 2.0
                and (company.get("payout_ratio", 0) or 0) <= 0.8
            ):
                passed.append(company)
        return passed
    ...
    def generate_regional_analysis(self, results: pd.DataFrame) -> dict:
        ...
        print("\n" + "=" * 80)
        print("REGIONAL ANALYSIS - 20 COUNTRY UNIVERSE")
        print("=" * 80)
        ...
        for region in results["region"].unique():
            ...
            print(
                f"{region:<20} {len(region_df):<15,} {tier1_count:<10} {tier2_count:<10} {avg_score:<12.1f}"
            )
        ...
        print("\n📊 BY COUNTRY (Top 10):")
        print(
            f"{'Country':<15} {'Companies':<15} {'Tier 1':<10} {'Avg Score':<12} {'Avg Price CAGR':<15}"
        )
        print("-" * 70)
        ...
        for country in results["country"].value_counts().head(10).index:
            ...
            print(
                f"{country:<15} {len(country_df):<15,} {tier1:<10} {avg_score:<12.1f} {avg_price:<15.1f}%"
            )
    ...
    def generate_final_report(self, results: pd.DataFrame):
        ...
        print("\n" + "=" * 80)
        print("FINAL GLOBAL SCREENING REPORT - 20 COUNTRIES")
        print("=" * 80)
        ...
        print("\n🏆 GLOBAL TOP 15:")
        print(
            f"{'Rank':<5} {'Ticker':<15} {'Country':<12} {'Score':<8} {'Revenue':<10} {'Capex':<10}"
        )
        print("-" * 70)
        ...
        for idx, (i, row) in enumerate(results.head(15).iterrows(), 1):
            ...
            print(
                f"{idx:<5} {row['ticker']:<15} {row['country']:<12} {row['expansion_score']:<8.0f} {rev:<10} {capex:<10}"
            )
    ...
    def deploy_global(self):
        ...
        print("\n" + "=" * 80)
        print("🌍 GLOBAL EXPANSION SCREENING - 20 COUNTRY DEPLOYMENT")
        print("=" * 80)
        print("   Scope: 50,000+ companies")
        print("   Timeline: Real-time (0.13s for 25,000)")
        print("   Status: LAUNCHING NOW")
        ...
        results.to_csv(
            f"/Users/umashankar/Downloads/code/python_files/global_screening_20countries_{timestamp}.csv",
            index=False,
        )

        # Save by country
        for country in results["country"].unique():
            country_df = results[results["country"] == country].sort_values(
                "expansion_score", ascending=False
            )
            country_df.to_csv(
                f"/Users/umashankar/Downloads/code/python_files/screening_{country}_{timestamp}.csv",
                index=False,
            )

        print("\n✅ GLOBAL DEPLOYMENT COMPLETE")
        print(f"   Processing time: {time.time() - self.start_time:.2f} seconds")
        print("   Files saved: global_screening_20countries_*.csv")
        print("   Country files: screening_[COUNTRY]_*.csv")
