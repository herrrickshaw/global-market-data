...
def main():
    parser = argparse.ArgumentParser(
        description="Full KRX scanner — Darvas + Piotroski + Coffee Can"
    )
    ...
    print(f"\n{'#'*60}")
    print("  FULL KOREA (KRX) MARKET SCAN")
    ...
    for stock_info in universe:
        ...
        trend = (
            ...
            if dist_ma and dist_ma > 5
                ...
                if dist_ma and dist_ma < -5
                else "Near 200MA (Consolidation)" if dist_ma else "Need 200+ days"
            )
        )
    ...
    if not args.no_scans and breakout_info:
        ...
        with ThreadPoolExecutor(max_workers=args.workers) as pool:
            futures = {
                pool.submit(fundamental_scan, code, sfx): (code, sfx) for code, sfx in breakout_info
            }
