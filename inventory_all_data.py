            ...
            try:
                ...
                if file_hash:
                    self.files_by_hash[file_hash].append(
                        {"path": str(relative_path), "size": size, "type": ext}
                    )

                self.files_by_extension[ext].append(
                    {"path": str(relative_path), "size": size, "hash": file_hash}
                )
    ...
    def _create_strategy(self) -> Dict:
        ...
        return {
            ...
            "expected_savings": {
                ...
                "git_repo_reduction_percent": (
                    (self.duplicate_size / self.total_size * 100) if self.total_size > 0 else 0
                ),
