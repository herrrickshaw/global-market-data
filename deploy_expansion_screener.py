...
class ScreenerDeployment:
    ...
    def _stage1_prefilter(self, companies: list) -> list:
        ...
        print(
            f"   └─ Rejected: {sum(failed.values()):,} ({sum(failed.values())/len(companies)*100:.1f}%)"
        )
    ...
    def generate_reports(self, results: pd.DataFrame):
        ...
        print(
            f"{'Rank':<5} {'Ticker':<8} {'Score':<8} {'Revenue':<10} {'Capex':<10} {'Price 5Y':<10}"
        )
        print("-" * 60)
        ...
        for idx, (i, row) in enumerate(results.head(20).iterrows(), 1):
            ...
            print(
                f"{idx:<5} {row['ticker']:<8} {row['expansion_score']:<8.0f} {rev:<10} {capex:<10} {price:<10}"
            )
    ...
    def create_deployment_summary(self, results: pd.DataFrame) -> dict:
        ...
        summary = {
            ...
            "top_10_candidates": results.head(10)[["ticker", "expansion_score", "tier"]].to_dict(
                "records"
            ),
        }
        return summary
    ...
    def deploy(self):
        ...
        print(
            f"   Pass rate: {self.results_summary['final_output']/self.results_summary['total_input']*100:.1f}%"
        )
        print("\n🎯 READY FOR:")
        print("   ✓ Portfolio construction")
        print("   ✓ Investment committee review")
        print("   ✓ Price correlation validation")
