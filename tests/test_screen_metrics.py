...
def test_metrics_from_series():
    ...
    df = pd.DataFrame({"Close": close, "High": close, "Low": close, "Open": close, "Volume": vol})
    m = sm._metrics(df)
    ...
    mf = pd.DataFrame(
        {
            ...
            "DMA50": [280, 90, 60],
            "DMA200": [250, 100, 70],
            "DMA50_prev": [240, 110, 65],
            "DMA200_prev": [250, 100, 70],
            "RSI": [70, 45, 25],
            "High52": [305, 200, 120],
            "Low52": [100, 78, 48],
            "PctFromHigh": [-1.6, -60, -58],
            "PctFromLow": [200, 2, 4],
            "VolSpike": [1.0, 1.0, 1.0],
            "Ret252": [180, -40, -50],
        }
    )
