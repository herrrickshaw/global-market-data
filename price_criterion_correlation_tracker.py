...
class PriceCriterionCorrelationTracker:
    ...
    def calculate_correlations(self):
        ...
        print("   Objective: Measure predictive power of each 11-D criterion")
        print("   Method: Spearman rank correlation (robust to outliers)")
        print("   Threshold: p-value < 0.05 = statistically significant")
        ...
        print(
            f"{'Criterion':<35} {'Correlation':<15} {'P-Value':<12} {'Significance':<15} {'Direction':<10}"
        )
        print("-" * 90)
        ...
        for col_name, (display_name, category) in criteria_map.items():
            ...
            print(
                f"{display_name:<35} {spearman_corr:>7.4f}  {spearman_p:>12.6f} {significance:<15} {direction:<10}"
            )
        ...
        self.correlations = pd.DataFrame(results_data).sort_values(
    ...
    def identify_signal_vs_noise(self):
        ...
        strong_signal = self.correlations[
            (abs(self.correlations["spearman_r"]) > 0.04) & (self.correlations["is_significant"])
        ]
        ...
        noise = self.correlations[abs(self.correlations["spearman_r"]) <= 0.02]

        print("\n🟢 STRONG SIGNAL CRITERIA (correlation > 0.04, p < 0.05):")
        print(f"   Count: {len(strong_signal)}")
        for idx, row in strong_signal.iterrows():
            print(
                f"   • {row['criterion']:35s} | r={row['spearman_r']:>7.4f} | p={row['spearman_p']:.4f}"
            )
        ...
        print("\n💡 RECOMMENDATIONS:")
        print("   ✓ Keep & increase weight: Strong signal criteria")
        print("   ⚠ Investigate: Weak signal criteria (may interact with others)")
        print("   ✗ Reduce/remove: Noise criteria (consider dropping)")
    ...
    def tier_price_performance(self):
        ...
        if len(df_tier_perf) > 1:
            ...
            if outperformance > 5:
                print("   ✓ Model VALIDATES: Higher tiers outperform by >5%")
            elif outperformance > 0:
                print("   ⚠ Weak validation: Outperformance <5%")
            else:
                print("   ✗ Model FAILS: Higher tiers underperform")
    ...
    def generate_recommendations(self):
        ...
        if len(strong) > 0:
            for idx, row in strong.iterrows():
                if row["criterion"] in ["Capex Acceleration CAGR", "FCF Margin"]:
                    print(
                        f"   ✓ {row['criterion']:35s} (r={row['spearman_r']:.4f}) - Already well-weighted"
                    )
                else:
                    print(
                        f"   ⚡ {row['criterion']:35s} (r={row['spearman_r']:.4f}) - Consider +5pp"
                    )
        ...
        if len(weak) > 0:
            ...
            print("   All criteria are either strong or noise")
        ...
        if len(noise) > 0:
            for idx, row in noise.iterrows():
                print(
                    f"   ✗ {row['criterion']:35s} (r={row['spearman_r']:.4f}) - Consider removing"
                )
        else:
            print("   No clear noise criteria identified")

        print("\n4. FURTHER INVESTIGATION NEEDED:")
        print("   • Test interaction effects (e.g., capex × ROIC)")
        print("   • Analyze by sector (criteria effectiveness varies by industry)")
        print("   • Test on global universe vs US-only")
        print("   • Backtest on historical periods (2015-2020 vs 2021-2026)")
...
if __name__ == "__main__":
    ...
    try:
        results = pd.read_csv(
            "/Users/umashankar/Downloads/code/python_files/expansion_screening_results_11d.csv"
        )
    except Exception:
