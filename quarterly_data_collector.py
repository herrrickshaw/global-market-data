...
class QuarterlyDataCollector:
                    ...
                    if qtr_is is not None and len(qtr_is.columns) > 0:
                        ...
                        if qtr_bs is not None and len(qtr_bs.columns) > 0:
                            ...
                            for qtr in quarters[:20]:  # Last 20 quarters
                                ...
                                try:
                                    metrics["revenue"] = (
                                        qtr_is.loc["Total Revenue", qtr]
                                        if "Total Revenue" in qtr_is.index
                                        else None
                                    )
                                    metrics["operating_income"] = (
                                        qtr_is.loc["Operating Income", qtr]
                                        if "Operating Income" in qtr_is.index
                                        else None
                                    )
                                    metrics["net_income"] = (
                                        qtr_is.loc["Net Income", qtr]
                                        if "Net Income" in qtr_is.index
                                        else None
                                    )
                                except Exception:
                                    metrics["revenue"] = metrics["operating_income"] = metrics[
                                        "net_income"
                                    ] = None
                                ...
                                try:
                                    metrics["ocf"] = (
                                        qtr_cf.loc["Operating Cash Flow", qtr]
                                        if "Operating Cash Flow" in qtr_cf.index
                                        else None
                                    )
                                    metrics["capex"] = (
                                        abs(qtr_cf.loc["Capital Expenditure", qtr])
                                ...
                                try:
                                    metrics["total_assets"] = (
                                        qtr_bs.loc["Total Assets", qtr]
                                        if "Total Assets" in qtr_bs.index
                                        else None
                                    )
                                    metrics["total_debt"] = (
                                        qtr_bs.loc["Total Debt", qtr]
                                        if "Total Debt" in qtr_bs.index
                                        else None
                                    )
                                    metrics["total_equity"] = (
                                        qtr_bs.loc["Total Stockholder Equity", qtr]
                                        if "Total Stockholder Equity" in qtr_bs.index
                                        else None
                                    )
                                except Exception:
                                    metrics["total_assets"] = metrics["total_debt"] = metrics[
                                        "total_equity"
                                    ] = None
    ...
    def collect_data(self, tickers: list) -> dict:
        ...
        print("\n" + "=" * 80)
        print("PHASE 1: QUARTERLY FUNDAMENTALS COLLECTION (5 YEARS)")
        print("=" * 80)

        print(f"\n🎯 COMPANIES TO ANALYZE: {len(tickers)}")
        print("   Data period: 5 years = 20 quarterly reports per company")
    ...
    def analyze_trends(self, collected: dict):
        ...
        print("   • Revenue trends (CAGR)")
        print("   • Operating income & margins")
        print("   • Capex acceleration (CAGR)")
        print("   • Debt trajectory (CAGR)")
        print("   • Operating cash flow")
        print("   • Free cash flow (OCF - Capex)")
        ...
        if expansion_scores:
            ...
            medium_expansion = len(
                df_scores[(df_scores["score"] >= 50) & (df_scores["score"] < 75)]
            )
...
if __name__ == "__main__":
    ...
    print("   Next: Phase 2 - Collect daily price data for correlation analysis")
