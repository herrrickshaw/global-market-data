...
class DailyPriceCollector:
    ...
    def collect_prices(self, tickers: list) -> dict:
        ...
        print("\n" + "=" * 80)
        print("PHASE 2: DAILY PRICE DATA COLLECTION (5 YEARS)")
        print("=" * 80)

        print(f"\n🎯 COMPANIES TO ANALYZE: {len(tickers)}")
        print(
            f"   Time period: {self.start_date.strftime('%Y-%m-%d')} to {self.end_date.strftime('%Y-%m-%d')}"
        )
        print("   Trading days per company: ~1,825 (5 years)")
        ...
        print(
            f"   Average records per company: {total_records // success_count if success_count > 0 else 0}"
        )
    ...
    def analyze_price_metrics(self, collected: dict):
        ...
        print("   • Daily Open, High, Low, Close, Volume")
        print("   • Adjusted Close (split & dividend adjusted)")
        print("   • Daily Returns (% change)")
        print("   • Log Returns (log-scale for modeling)")
        print("   • Price Momentum (20-day & trend)")
        print("   • Volatility (20-day rolling std)")
        ...
        if returns_list:
            ...
            print(
                f"      Positive days ratio: {sum(1 for r in returns_list if r > 0) / len(returns_list) * 100:.1f}%"
            )
    ...
    def prepare_for_correlation(self, price_data: dict) -> pd.DataFrame:
        ...
        print("      ✓ 5-year price CAGR")
        print("      ✓ Total return")
        print("      ✓ 1-year & 2-year momentum")
        print("      ✓ Volatility (1-year & 3-year)")
        print("      ✓ Sharpe ratio (risk-adjusted return)")
        print("      ✓ Maximum drawdown")

        if not df_corr.empty:
            print("\n   Top performers by 5-year price CAGR:")
            top_performers = df_corr.nlargest(5, "price_5yr_cagr")[
                ["ticker", "price_5yr_cagr", "volatility_1y", "sharpe_ratio"]
            ]
            for idx, row in top_performers.iterrows():
                print(
                    f"      {row['ticker']:6s} - CAGR: {row['price_5yr_cagr']:6.1f}% | Vol: {row['volatility_1y']:5.1f}% | Sharpe: {row['sharpe_ratio']:5.2f}"
                )
...
if __name__ == "__main__":
    ...
    print("   Ready for: Phase 3 - Correlation analysis (price vs quarterly metrics)")
    print("   Ready for: Phase 4 - Backtest (8-D vs 11-D model weights)")
